...
def main():
    ...
    # messagebox.showinfo("Credit", "This was programmed by Habibor rahaman 🤪")
...
def start_app():
    input = input_box.get()
    if not input:
        messagebox.showwarning("No Input", "Enter some keyword first")
        return 1
...
def algo_1(words, list):
    password = str(random.randint(0, 9)) * random.randint(0, 5) + str("".join(words)) + str(
